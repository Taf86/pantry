...
[[ -r $TAG_FILE ]] || { log "ERROR: $TAG_FILE missing or invalid"; exit 1; }
...
if [[ -z $TAG_OLD ]]; then
  log "no previous version to roll-back: stack remains unchanged"
  exit 1
fi
...
if compose up -d --wait --wait-timeout "$WAIT_TIMEOUT" --remove-orphans; then
  log "rollback succeeded: running $TAG_OLD"
else
